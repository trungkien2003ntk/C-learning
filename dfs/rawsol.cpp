...
// START PROGRAM
//============================================================================
int n, m, size_path = INT_MAX, amount = 0;

struct node_type
{
    bool visited = 0;
    vector<int> destination, distance;
};
...
void nhap()
{
    ...
    {
        cin >> u >> v >> l;
        node[u].destination.PB(v);
        node[u].distance.PB(l);
        node[v].destination.PB(u);
        node[v].distance.PB(l);
    }
    fclose(stdin);
}
...
void dfs(int x, int sz)
{
    ...
    if (x == n)
    {
        ...
        for (int i = 0; i < node[x].destination.size(); i++)
            if (!node[node[x].destination[i]].visited)
                dfs(node[x].destination[i], sz + node[x].distance[i]);
    node[x].visited = 0;
}

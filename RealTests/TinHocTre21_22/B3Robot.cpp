...
void tracking(int x, int y)
{
    ...
    visited[x][y] = amount;
    if (x - 1 >= 0 && !visited[x - 1][y] && bg[x - 1][y] == val[amount])
        tracking(x - 1, y);
    if (x + 1 < N && !visited[x + 1][y] && bg[x + 1][y] == val[amount])
        tracking(x + 1, y);
    if (y + 1 < N && !visited[x][y + 1] && bg[x][y + 1] == val[amount])
        tracking(x, y + 1);
    if (y - 1 >= 0 && !visited[x][y - 1] && bg[x][y - 1] == val[amount])
        tracking(x, y - 1);
